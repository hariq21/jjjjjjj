...
export default function GameDetail({ params }: { params: { id: string } }) {
  ...
  const handleQris = async (diamond: any) => {
    ...
    try {
      ...
      console.log(result);
      alert("Response QRIS: " + JSON.stringify(result));
    } catch (err) {
    ...
    <main className="min-h-screen bg-gray-50">
      <div className="max-w-4xl mx-auto px-6 py-12">
        <div className="bg-blue-600 text-white rounded-2xl shadow-md p-10 text-center mb-10">
          ...
          <p className="mt-2 text-lg text-blue-100">Masukkan data akun dan pilih jumlah diamond</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {diamonds.map((diamond) => (
            <div key={diamond.id} className="bg-white rounded-xl shadow-lg p-6 text-center hover:scale-105 transition">
              <h3 className="text-lg font-bold mb-2">{diamond.name}</h3>
              <p className="text-gray-600 mb-4">Rp {diamond.price.toLocaleString()}</p>
              <button
